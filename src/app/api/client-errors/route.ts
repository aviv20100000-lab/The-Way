...
export async function POST(req: NextRequest) {
  ...
  if (isPerfReport) {
    ...
    const details = [
      ...
      `Nav type: <code>${escapeHtml(clean(body.navigationType, 30) || "unknown")}</code>`,
      timing("Activation start ms", body.activationStart),
      timing("Wall gap ms", body.wallGapMs),
      timing("Module init perf ms", body.moduleInitPerf),
      timing("Body text length at init", body.bodyTextLenAtInit, 10_000_000),
      timing("Body children at init", body.bodyChildrenAtInit, 10_000),
