...
import { getDayRangeUtc, getTodayDayKey, getYesterdayDayKey } from "@/lib/daily-summary";
...
export async function POST(req: NextRequest) {
  ...
  const todayRange = getDayRangeUtc(today);
  const [goalRes, totalRes] = await Promise.all([
    db.execute({ sql: "SELECT daily_water_ml FROM goals WHERE user_id = ?", args: [user.id] }),
    db.execute({
      sql: "SELECT COALESCE(SUM(amount_ml), 0) as total FROM water_logs WHERE user_id = ? AND logged_at >= ? AND logged_at < ?",
      args: [user.id, todayRange.startUtc, todayRange.endUtc],
    }),
  ]);
